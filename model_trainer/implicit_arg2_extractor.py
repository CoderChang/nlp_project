...
class Implicit_arg2_extractor(object):
    ...
    def train_model(self, pdtb_data_file, pdtb_parses_file):
        if WRITE_EXAMPLE_FLAG and os.path.exists(config.TRAIN_EXAMPLES_IMPLICIT_ARG2_TRAIN):
            print 'using existed training examples ...'
            train_examples = util.load_examples_from_file(config.TRAIN_EXAMPLES_IMPLICIT_ARG2_TRAIN)
            ...
            if WRITE_EXAMPLE_FLAG:
                util.write_examples_to_file(config.TRAIN_EXAMPLES_IMPLICIT_ARG2_TRAIN, train_examples)

            print 'yes_num: ', yes_num, ' no_num: ', no_num
    ...
    def extract_argument(self, doc, non_explicit_relations):
        for relation in non_explicit_relations:
            ...
            if arg2_token_list == []:
                ...
                #arg2_token_list = relation['Arg2']['TokenList']
            else:
                arg2_token_list = range(min(arg2_token_list), max(arg2_token_list)+1)
                #arg2_token_list = [ [-1, -1, -1, sent_index, ind] for ind in range(min(arg2_token_list), max(arg2_token_list)+1)]
    ...
    def test_model(self, pdtb_data_file, pdtb_parses_file):
        if WRITE_EXAMPLE_FLAG and os.path.exists(config.TRAIN_EXAMPLES_IMPLICIT_ARG2_TEST_TEST) and os.path.exists(config.TRAIN_EXAMPLES_IMPLICIT_ARG2_TEST_TRAIN):
            print 'using existed test examples ...'
            test_examples = util.load_examples_from_file(config.TRAIN_EXAMPLES_IMPLICIT_ARG2_TEST_TEST)
            test_train_examples = util.load_examples_from_file(config.TRAIN_EXAMPLES_IMPLICIT_ARG2_TEST_TRAIN)
            ...
            if WRITE_EXAMPLE_FLAG:
                util.write_examples_to_file(config.TRAIN_EXAMPLES_IMPLICIT_ARG2_TEST_TEST, test_examples)
                util.write_examples_to_file(config.TRAIN_EXAMPLES_IMPLICIT_ARG2_TEST_TRAIN, test_train_examples)
...
if __name__ == '__main__':
    ...
    classifier.load_model_from_file(config.TRAIN_MODEL_IMPLICIT_ARG2_EXTRACTOR)
